...
{ 
		...
		int pro=0, que=0, now=0, best=0, err=0, end=0, byte=0, right=0, i=0, j=0;
		char input=' ';
		float acc=100;
		time_t start=0, finish=0;
		// pro 진행도, que 문제선택, now 현재타수, best 최고타수, err 오타, end 메인복귀확인, byte 입력한 글자수, right 맞은 글자수
		// acc 정확도,
		// start 시작시간, end 끝시간
		...
		while(pro<5)
		{
				...
				else if(input == 127) //backspace
				{
						...
						{
							byte--;
							if(sentence[que][i-1]==ans[i-1])//right 하나 줄여주기
									right--;
							ans[i-1]='\0';
							i--;
						}
